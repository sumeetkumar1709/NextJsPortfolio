...
function About({pageInfo}: Props) {
  return (
    <motion.div initial={{opacity:0}} whileInView={{opacity:1}} transition={{duration:1.5}} className='h-screen flex relative flex-col text-center md:text-left md:flex-row max-w-7xl px-10 justify-evenly mx-auto items-center'>
        ...
        <motion.img initial={{x:-200,opacity:0,}} transition={{duartion:1.2,}} whileInView={{opacity:1,x:0}} viewport={{once:true}} className='-mb-20 md:mb-0 flex-shrink-0 w-56 h-56 object-cover  md:w-64 md:h-95 xl:w-[500px] xl:h-[600px]' src={urlFor(pageInfo.profilePic).url()} alt=""/>   
